...
function movePaddle() {
	var Left = document.getElementById('pressLeft');
	Left.addEventListener("mousedown", downL, false);
	Left.addEventListener("mouseup", upL, false);

	var Right = document.getElementById('pressRight');
	Right.addEventListener("mousedown", downR, false);
	Right.addEventListener("mouseup", upR, false);

	function downL(e) {
	  if(e) { setting.paddleMoveLeft = true; }
	}
	function upL(e) {
	  if(e) { setting.paddleMoveLeft = false; }
	}

	function downR(e) {
	  if(e) { setting.paddleMoveRight = true; }
	}
	function upR(e) {
	  if(e) { setting.paddleMoveRight = false; }
	}
}
...
function initSpacebar() {
	var but = document.getElementById('space');
	$(but).on('click', function(e) {
		if( e && setting.deathNumber < setting.deathTotal ) {
			gameStart = setInterval( init , setting.frameSpeed);
			$('.instr-footer small').text('');
		}
	});
